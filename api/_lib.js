/* =====================================================================
   CraveNG â€” server-side helpers  (runs ONLY on Vercel, never in a browser)
   ---------------------------------------------------------------------
   ...
   begins with NEXT_PUBLIC_ / VITE_ â€” none of these do, deliberately.
   ===================================================================== */
...
export const ENV = {
  ...
  template:   process.env.WHATSAPP_TEMPLATE_NAME     || '',   // blank = plain text
  templateLang: process.env.WHATSAPP_TEMPLATE_LANG   || 'en',

  // --- test mode: log the message instead of calling Meta ---
  testMode:  String(process.env.WHATSAPP_TEST_MODE || 'true').toLowerCase() !== 'false',

  // --- signing secret for vendor action links ---
  linkSecret: process.env.ORDER_LINK_SECRET || '',

  // --- Supabase, server side ---
  sbUrl:      process.env.SUPABASE_URL || '',
  sbService:  process.env.SUPABASE_SERVICE_ROLE_KEY || '',
...
/* Supabase has two secret-key formats and they must be sent differently.
   ...
   New secret key ("sb_secret_..." â€” NOT a JWT):
       ...
       error "permission denied for table cng_orders" (42501) â€” an
...
export function serviceHeaders(extra){
  const h = {
    apikey: ENV.sbService,
    'Content-Type': 'application/json',
  };
  if(isJwtKey(ENV.sbService)) h.Authorization = 'Bearer ' + ENV.sbService;
...
export function keyKind(){
  ...
  if(k.startsWith('sb_publishable_')) return 'PUBLISHABLE (wrong key â€” this cannot write)';
  return 'unrecognised';
}
...
async function sb(path, opts = {}){
  ...
  if(!r.ok){
    ...
    if(code === '42501' || /permission denied/i.test(msg)){
      msg = 'permission denied â€” the server is not authenticating as service_role. '
          + 'SUPABASE_SERVICE_ROLE_KEY looks like: ' + keyKind() + '. '
          + 'Use the secret/service_role key from Supabase â†’ Settings â†’ API, not the publishable key.';
    }
...
/* ---------------------------------------------------------------------
   ...
   usually stored as 080â€¦, which must become 23480â€¦.
   --------------------------------------------------------------------- */
export function toE164(raw, countryCode = '234'){
  let d = String(raw || '').replace(/[^\d+]/g, '');
  if(d.startsWith('+')) d = d.slice(1);
  if(d.startsWith('00')) d = d.slice(2);
  if(d.startsWith('0')) d = countryCode + d.slice(1);        // 0803â€¦ -> 234803â€¦
