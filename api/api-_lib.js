...
export function keyKind(){
  ...
  if(k.startsWith('sb_publishable_')) return 'PUBLISHABLE (wrong key — this cannot write)';
  return 'unrecognised';
}
...
async function sb(path, opts = {}){
  ...
  if(!r.ok){
    ...
    if(code === '42501' || /permission denied/i.test(msg)){
      msg = 'permission denied — the server is not authenticating as service_role. '
          + 'SUPABASE_SERVICE_ROLE_KEY looks like: ' + keyKind() + '. '
          + 'Use the secret/service_role key from Supabase → Settings → API, not the publishable key.';
    }
